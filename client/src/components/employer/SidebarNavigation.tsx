...
import { navItems } from "@/constants/sidebarLeft";
  ...
  return (
    ...
    >
      {navItems.map((navItem, index) => (
        ...
        >
          <Tooltip
            key={`${navItem.id}-${isSidebarLeftExpanded}-${showTooltips}`}
          >
            <TooltipTrigger asChild>
              <Button
                variant={currentPage === navItem.id ? "default" : "ghost"}
                className={cn(
                  "h-11 rounded-xl text-primary hover:text-primary",
                  isSidebarLeftExpanded
                    ? "justify-start gap-3 w-full px-6!"
                    : "justify-center gap-0 w-12 px-0",
                  currentPage === navItem.id &&
                    "bg-primary text-primary-foreground hover:text-primary-foreground shadow-md",
                )}
